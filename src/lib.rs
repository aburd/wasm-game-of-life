    ...
    fn live_neighbor_count(&self, row: u32, column: u32) -> u8 {
        ...
        for delta_row in [self.height - 1, 0, 1].iter() {
            for delta_column in [self.width - 1, 0, 1].iter() {
                ...
                let neighbor_index = self.get_index(neighbor_row, neighbor_column);
        ...
        for row in 0..self.height {
            for column in 0..self.width {
                ...
                let next_cell = match (cell, live_neighbors) {
                    ...
                    (Cell::Alive, neighbors) if (neighbors == 2 || neighbors == 3) => Cell::Alive,
                    // Rule 3: Any live cell with more than three live
                    // neighbours dies, as if by overpopulation.
                    (Cell::Alive, neighbors) if neighbors > 3 => Cell::Dead,
                    // Rule 4: Any dead cell with exactly three live neighbours
                    // becomes a live cell, as if by reproduction.
                    (Cell::Dead, neighbors) if neighbors == 3 => Cell::Alive,
